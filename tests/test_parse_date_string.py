...
@pytest.mark.parametrize('date_string, expected_parse_arg, expected_captures, expected_date', [
    ...
    (' on 11-20-2015 6pm CST ', # test an abbreviation that tz.gettz will return None for
     '11-20-2015 6pm',
     {'timezones':['CST']},
     datetime(2015, 11, 20, 18, 0) # we're making sure it's naive
    ),
    (' on 11-20-2015 6am IRST ', # test an abbreviation that tz.gettz will return None for
     '11-20-2015 6am',
     {'timezones':['IRST']},
     datetime(2015, 11, 20, 6, 0) # we're making sure it's naive
    )
])
